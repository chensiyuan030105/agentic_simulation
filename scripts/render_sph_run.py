...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render an SPH run with Blender.")
    parser.add_argument("run_dir", type=Path, help="SPH run directory containing sph_scene.json and particles.npz.")
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    runtime_config_path = run_dir / "render_sph_config.runtime.json"
    ...
    command = [
        ...
        "1",
        "--python",
        str(REPO_ROOT / "blender" / "render_sph.py"),
        "--",
        "--config",
